...
def fetch_resources(db_name, team_name, mongodb_uri, action=None, cpu=None, memory=None):
    try:
        ...
        # Fetch team data
        team_data = collection.find_one({"team_name": team_name})
        ...
        # If update action is provided, update the values
        if action == "update" and cpu and memory:
            collection.update_one(
                {"team_name": team_name},
                {"$set": {"Allocated_CPU": int(cpu), "Allocated_Memory": int(memory)}}
            )
            return {"message": f"Updated CPU to {cpu} and Memory to {memory}GB"}

        return {"allocated_cpu": allocated_cpu, "allocated_memory_gb": allocated_memory_gb}
